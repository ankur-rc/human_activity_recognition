...
def run_cnn(x_train,y_train,x_test,y_test,prev_accuracy):
  ...
  # create cnn model
  model = Sequential()
  model.add(Conv1D(filters=32, kernel_size=5, activation='relu', dilation_rate=1, input_shape=(samples,features)))
  model.add(BatchNormalization())
  model.add(Conv1D(filters=32, kernel_size=5, activation='relu', dilation_rate=2))
  model.add(BatchNormalization())
  model.add(Conv1D(filters=32, kernel_size=5, activation='relu', dilation_rate=4))
  model.add(Conv1D(filters=32, kernel_size=5, activation='relu', dilation_rate=8))
