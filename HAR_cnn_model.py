...
def run_cnn(x_train,y_train,x_test,y_test,prev_accuracy):
  ...
  model.add(Conv1D(filters=32, kernel_size=3, activation='relu', dilation_rate=1, input_shape=(samples,features)))
  model.add(Dropout(0.2))
  model.add(Conv1D(filters=32, kernel_size=3, activation='relu', dilation_rate=2))
  model.add(Dropout(0.2))
  model.add(Conv1D(filters=32, kernel_size=3, activation='relu', dilation_rate=4))
  model.add(Dropout(0.2))
  model.add(Conv1D(filters=32, kernel_size=3, activation='relu', dilation_rate=8))
  model.add(Dropout(0.2))
